...
populateRadios('drying', formUniqueOptions('drying'));
populateSelect('country', formUniqueOptions('country'));
populateSelect('width', formUniqueOptions('width'));
...
onChangeHandler('.drying input', 'drying');
onChangeHandler('#country', 'country');
onChangeHandler('#width', 'width');
...
function filterProducts() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    if (state.drying !== '') {
      isCorrect.push(state.drying === products[i].drying);
    }
    if (state.country !== '') {
      isCorrect.push(state.country === products[i].country);
    }
    if (state.width !== '') {
      isCorrect.push(state.width === products[i].width);
    }
...
function renderProducts(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const type = $('<div>', { text: `Загрузка: ${product.type}` });
    const category = $('<div>', { text: 'Бренд: ' + product.category });
    const color = $('<div>', { text: `Цвет: ${product.color}` });
    const parameter = $('<div>', { text: `Вместимость: ${product.parameter}` });
    const drying = $('<div>', { text: `Сушка: ${product.drying}` });
    const country = $('<div>', { text: 'Страна: ' + product.country });
    const width = $('<div>', { text: 'Ширина: ' + product.width });
    item
      .append(img)
      .append(name)
      .append(type)
